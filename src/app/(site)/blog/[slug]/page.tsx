...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let timeFormatted = '';
  try {
    dateFormatted = dateObj.toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric', timeZone: 'America/Sao_Paulo' });
    timeFormatted = dateObj.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' });
  } catch {
    dateFormatted = dateObj.toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' });
    timeFormatted = dateObj.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
  }
        ...
        <div className="blog-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 40, alignItems: 'start' }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            {(() => {
              ...
              // Se só encontrou 1 time no título, tenta buscar o segundo time no resumo ou conteúdo do post
              if (home && !away) {
                const textSearch = (postResumo || '') + ' ' + (rawContent || '');
                const extraTeams = extractTeamsFromTitle(textSearch);
                if (extraTeams.home && extraTeams.home.name !== home.name) {
                  away = extraTeams.home;
                } else if (extraTeams.away && extraTeams.away.name !== home.name) {
                  away = extraTeams.away;
                }
              }

              return <TeamMatchBanner home={home} away={away} ctaUrl={activeCta.url_destino} />;
            })()}
